...
function CalendarView() {
  ...
  const handleSave = useCallback((post) => {
    if (modalMode === 'create') addPost({ ...post, id: crypto.randomUUID(), preferredStart: post.start });
    else updatePost(post);
    closeModal();
  }, [addPost, closeModal, modalMode, updatePost]);
  ...
  const handleDragStart = useCallback(({ event }) => setScheduleInsight(getSchedulingInsight(event, event.start, posts)), [posts, setScheduleInsight]);
